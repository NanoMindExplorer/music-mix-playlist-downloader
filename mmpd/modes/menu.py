"""
Mode 1, 4, 5: Interactive menu loop (Fase A — dipecah dari download.py).

Berisi semua UI interaktif:
- Main menu loop (run_cli)
- Konfigurasi unduhan (prompt target/format/lyrics/transliterasi)
- Tabel konfigurasi ringkasan
- Post-download lyrics processing untuk SEMUA audio (mode interaktif)

Logika non-Spotify-spesifik ada di sini; helper ISRC/concurrent Spotify
ada di mmpd/modes/spotify_download.py; runner non-interaktif (CLI) tetap
diekspos dari mmpd/modes/download.py.
"""
...
# Fase A: helper Spotify (ISRC matching + concurrent) pindah ke modul sendiri
from mmpd.modes.spotify_download import (
...
from mmpd.ytdlp import build_download_opts, default_download_hook
