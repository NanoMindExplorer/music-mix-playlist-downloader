...
🎵 Panduan Pemutar Musik yang Mendukung Cover Art & Lirik Sinkron (.lrc)
...
- Download dari Play Store: Poweramp Music Player
- Buka Poweramp → Settings →
- Pastikan folder musik Anda di-scan.
- Aktifkan Lyrics dengan cara: Buka lagu → ketuk ikon "Lirik" di pojok kanan bawah.
...
- Download dari Play Store: Musicolet
- Buka app → izinkan akses penyimpanan.
- Buka Settings →
- "Lyrics" → Aktifkan "Show lyrics".
- "Lyrics source" → Pilih "Local .lrc files".
...
- Pastikan opsi sinkronisasi Huawei aktif saat download/retrofit.
- Buka Huawei Music app.
Folder struktur:
📁 Internal/Music/
├── 🎵 Song1.mp3
├── 🎵 Song2.mp3
...
- Settings → Lyrics → Aktifkan "Synced Lyrics".
- Letakkan file .lrc di folder yang sama dengan musik.
...
- Download dari musicbee.com.
- Buka MusicBee → Preferences →
- "Library" → Add folder musik Anda.
- "Tags" → Pastikan "Embed artwork" dicentang.

Aktifkan Lirik:
- Preferences → "Layout" (1) → pilih layout yang punya panel "Lyrics".
- Preferences → "Lyrics" →
- "Save lyrics to music file" → centang.
- "Lyrics file naming" → set ke %filename%.lrc.
Struktur file:
📁 D:\Music\
...
- Download dari foobar2000.org.
- Install plugin ESLyric:
- Download ESLyric dari forum foobar2000.
- Ekstrak ke folder foobar2000/components/.
- Buka foobar2000 →
- File → Preferences → Display → Columns UI →
- Tambahkan panel "ESLyric" ke layout.
- ESLyric akan otomatis baca file .lrc di folder yang sama dengan musik.


3. VLC Media Player
VLC bisa memutar audio dengan lirik sinkron via plugin.

Cara Menerapkan:

- Download VLC dari videolan.org.
- Buka file musik (.mp3) di VLC.
- Drag & drop file .lrc ke jendela VLC, atau:
- Klik menu "Subtitle" → "Add Subtitle File" → pilih file .lrc.
- Lirik akan muncul di atas video (VLC akan menampilkan visualizer audio + lirik).
...
Buka Sayonara → Settings →
"Library" → Add folder musik Anda.
Aktifkan Lirik:
Settings → "Lyrics" →
Aktifkan "Show lyrics".
"Lyrics server" → pilih "Local".
Struktur file :
📁 ~/Music/
├── 🎵 Adele - Hello.mp3
└── 📄 Adele - Hello.lrc
